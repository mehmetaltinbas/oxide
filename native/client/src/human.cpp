...
/**
 * The body is drawn in its own frame - origin at the chest, facing up the
 * screen - and placed by this, so the maths above reads as a drawing and not
 * as trigonometry.
 */
struct Frame {
    float ox;
...
void limb(Paint& paint, const Frame& f, float x0, float y0, float x1, float y1, float width,
          Color color) {
...
void blob(Paint& paint, const Frame& f, float x, float y, float rx, float ry, Color color,
          bool inked) {
...
void drawHuman(Paint& paint, const HumanLook& look) {
    ...
    const Frame f{look.x, look.y, std::cos(look.facing + 1.57079632679f),
    ...
    if (!look.swimming) {
        blob(paint, f, -4.6f, -step + 1, 3, 4.6f, shade(look.legs), true);
        blob(paint, f, 4.6f, step + 1, 3, 4.6f, shade(look.legs), true);
    }
