...
export class Hud {
    ...
    private drawMessage(game: Game, w: number): void {
        if (!game.message) return;
        this.ctx.font = '13px ui-monospace, monospace';
        const tw = this.ctx.measureText(game.message).width + 32;
        this.ui.glass(w / 2 - tw / 2, 62, tw, 30, RADIUS.sm);
        this.ui.textOnDark(game.message, w / 2, 71, { size: 'body', align: 'center' });
    }
